"""
DNA-FusionNet: Early Fusion Version (极简版)

策略：站在 DNANet 巨人的肩膀上，最小化改动，最大化收益。

核心改动：
    ✅ 唯一修改：将输入通道从 1 改为 2（红外 + LiDAR 深度）
    ✅ Early Fusion：让网络自己学习如何组合双模态信息
    ✅ 保持 DNANet 的密集嵌套结构（Dense Nested Attention）完全不变
    ✅ 保持深度监督（Deep Supervision）不变
...
预期效果：
    ...
    - FPR (误报率) 预期下降 20-40%（LiDAR 作为几何先验）

作者：DNA-FusionNet Project
日期：2026-01-29
"""
...
class DNAFusionNet_EarlyFusion(nn.Module):
    """
    DNA-FusionNet: Early Fusion Version

    相比原版 DNANet 的唯一改动：
        - input_channels: 1 → 2 (红外 + LiDAR)
        - 第一层卷积自动学习如何组合两个通道

    Args:
        num_classes: 输出类别数（二分类任务为 1）
        input_channels: 输入通道数（默认 2：红外 + LiDAR）
        block: 残差块类型（默认 Res_CBAM_block）
        num_blocks: 每层的残差块数量 [2, 2, 2, 2] for ResNet-18 style
        nb_filter: 每层的通道数 [16, 32, 64, 128, 256]
        deep_supervision: 是否使用深度监督（默认 True）
    """
    def __init__(self, num_classes, input_channels=2, block=Res_CBAM_block,
                 num_blocks=[2, 2, 2, 2], nb_filter=[16, 32, 64, 128, 256],
                 deep_supervision=True):
        super(DNAFusionNet_EarlyFusion, self).__init__()
        self.relu = nn.ReLU(inplace = True)
    ...
    def forward(self, input):
        ...
        Returns:
            如果 deep_supervision=True: [output1, output2, output3, output4]
            如果 deep_supervision=False: output
        """
        x0_0 = self.conv0_0(input)  # ✅ Early Fusion: 两个通道在此融合
...
# 便捷构造函数
def dnafusion_early_resnet18(num_classes=1, input_channels=2, deep_supervision=True):
    """
    DNA-FusionNet Early Fusion with ResNet-18 backbone

    适用场景：
        - 红外 + LiDAR 双模态船只检测
        - 快速验证 Early Fusion 效果
        - 训练资源有限

    配置：
        - Backbone: ResNet-18 style (2-2-2-2 blocks)
        - Filters: [16, 32, 64, 128, 256] (channel_size='three')
        - Deep Supervision: True（4 个输出头）
    """
    return DNAFusionNet_EarlyFusion(
...
if __name__ == '__main__':
    # 测试模型
    print("="*80)
    print("DNA-FusionNet Early Fusion - 模型测试")
    print("="*80)

    model = dnafusion_early_resnet18(num_classes=1, input_channels=2, deep_supervision=True)

    # 模拟输入：红外 + LiDAR
    dummy_input = torch.randn(2, 2, 512, 512)  # [B=2, C=2, H=512, W=512]
