...
#
# dataset: NUAA-SIRST | NUDT-SIRST | IRSTD-1k
# cmd:     prepare | train | test <ckpt> | export [results.pkl] | eval [pred_dir]
#
# Examples:
#   bash baselines/boxinstseg/run_boxlevelset.sh NUAA-SIRST train
#   bash baselines/boxinstseg/run_boxlevelset.sh NUDT-SIRST train
#   bash baselines/boxinstseg/run_boxlevelset.sh IRSTD-1k   train
#   bash baselines/boxinstseg/run_boxlevelset.sh NUAA-SIRST test work_dirs/.../epoch_36.pth
...
if [[ -z "$DATASET" || -z "$CMD" ]]; then
  echo "Usage: bash $0 <NUAA-SIRST|NUDT-SIRST|IRSTD-1k> <prepare|train|test|export|eval> [args]"
  exit 1
fi
...
case "$DATASET" in
  ...
  IRSTD-1k)
    ...
    ;;
  *)
    echo "Unknown dataset: $DATASET. Choose from NUAA-SIRST, NUDT-SIRST, IRSTD-1k" >&2
    exit 1
    ;;
esac
